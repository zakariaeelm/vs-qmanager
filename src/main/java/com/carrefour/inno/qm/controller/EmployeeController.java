...
public class EmployeeController {
    ...
    public EmployeeController(EmployeeDAO employeeDao, RestTemplate restTemplate, CovidService covidService) {
    ...
    public Store customersCount(@PathVariable("store") String store,
                                @PathVariable("date") String date) {

        PhenixTrxResponse trx = getTrxCount(store, date);
        logger.info(trx.toString());
        System.out.println(date);
        Random r = new Random();
        int letIn = r.nextInt((30 - 10) + 1) + 10;
        return covidService.computeDiff(store, trx);
    }
    ...
    {
        return covidService.findOneByPpsf(ppsf.toUpperCase());
    }
